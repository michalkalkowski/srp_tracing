...
- single material anisotropic (transversely isotropic) with the grain growth
axis vertical.
...
# Set all the weld pixels to one defined orientation of -30 deg
weld.grain_orientations[:] = np.deg2rad(-30)
input_orientations = np.copy(weld.grain_orientations)
# Model geometry
length = 110
abs_width = 20
top_height = a
...
model.define_field_save_increment(total_steps=50,
                                  field_store_nodes=-1)
pw.write_input_file(model, 'SRP_validation_anisotropic.pogo-inp')
