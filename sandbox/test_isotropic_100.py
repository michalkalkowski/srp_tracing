...
c_iso = misc.young2C(210e9, 0.3).real
rho_iso = 7.9e3

parent_basis = grid.WaveBasis(anisotropy=0)
parent_basis.set_material_props(c_iso, rho_iso)
parent_basis.calculate_wavespeeds()

devs = np.arange(0.1, 0.5, 0.05)
...
time_theoretical = abs_dist.squeeze()/parent_basis.wavespeed**0.5

# Plot results
time_srp = np.array(time_srp)
...
# Relative error
fig, ax = plt.subplots()
ax.plot(devs, np.nanmean(relative*100, axis=(2, 3)), '-o')
...
# Absolute error
fig, ax = plt.subplots()
ax.plot(devs, 1e6*np.nanmean(absolute, axis=(2, 3)), '-o')
ax.set_xlabel('grid dev')
