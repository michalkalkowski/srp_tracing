...
with a non-planar backwall.

An austenitic stainless steel weld with an Ogilvy orientation map.
Sources and receivers positioned centrally on the top surface of
...
The backwall profile is loaded from an external file and graph edges between the nodes at
the backwall are linked with zero cost.
...
gf = h5.File('../data/geometry2GGA.h5', 'r')
right_chamfer = gf['right'][:]
left_chamfer = gf['left'][:]
top_surface = gf['top'][:]
bottom_surface = gf['bottom'][:]
gf.close()
...
backwall = interp1d(bottom_surface[:, 0], bottom_surface[:, 1], kind='linear')
gridx = np.unique(test_grid.grid_1[:, 0])
backwall_grid = backwall(gridx)

temp_grid = np.copy(test_grid.grid_1)
taken = []
for i in range(backwall_grid.shape[0]):
    take = np.where(np.isclose(temp_grid[:, 0], gridx[i]) &
                   (temp_grid[:, 1] > backwall_grid[i]))[0]
    taken.extend(list(take))
taken = np.unique(np.array(taken).flatten())

temp_grid = np.concatenate((temp_grid[taken],
                            np.column_stack((gridx, backwall_grid))), axis=0)
mirror_grid = np.copy(temp_grid)
mirror_grid[:, 1] *= -1

test_grid.grid_1 = np.r_[temp_grid, mirror_grid]
...
backwall1 = []
for i in range(backwall_grid.shape[0]):
    take = np.where(np.isclose(test_grid.grid[:, 0], gridx[i]) &
                   np.isclose(test_grid.grid[:, 1], backwall_grid[i]))[0]
    backwall1.extend(list(take))

backwall1 = np.unique(np.array(backwall1))
backwall2 = []
for i in range(backwall_grid.shape[0]):
    take = np.where(np.isclose(test_grid.grid[:, 0], gridx[i]) &
                   np.isclose(test_grid.grid[:, 1], -backwall_grid[i]))[0]
    backwall2.extend(list(take))
backwall2 = np.unique(np.array(backwall2))


test_grid.calculate_graph(tie_link=[backwall1, backwall2])
...
target = np.load('../data/SRP_validation_backwall_ogilvy_rough.npy')
