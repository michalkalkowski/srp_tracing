...
An austenitic stainless steel weld with constant orientations across the weld.
...
# initial map
weld.grain_orientations = np.deg2rad(-30)*np.ones(
    weld.grain_orientations.shape)
weld.grain_orientations_full[weld.in_weld == 1] = weld.grain_orientations
# Define the domain
...
tofs_srp[32:, 32:] = np.nan
target = np.load('../references/FE_models/results/SRP_validation_anisotropic.npy')# - 0.75e-6
target[:32, :32] = np.nan
